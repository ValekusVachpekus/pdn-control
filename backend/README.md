...
⚠️ **Краулер сейчас медленный — ~30 c на страницу.** Playwright на каждой странице
ждёт `networkidle` вплоть до `page_timeout_ms=20000`, а на сайтах с аналитикой/
long-poll соединения не «затихают» — и он выжидает весь таймаут. Из-за этого обход
крупного сайта на большом числе страниц вылетает за `SCAN_TIMEOUT_SEC=300` →
краулер отдаёт `504` → бэк помечает скан `failed`
(симптом: `crawler 504: Проверка превысила лимит 300 c`).

**Сделано (быстрый фикс):** `PAID_MAX_PAGES=20` в `.env.secret` — обход укладывается
в лимит. Для технического аудита 152-ФЗ нескольких десятков публичных страниц
достаточно.

**TODO (лечение первопричины, в `../crowler`):** ускорить загрузку страницы —
снизить `page_timeout_ms` (20000 → ~8000) и сменить стратегию ожидания на
`wait_until="domcontentloaded"` вместо `networkidle`
(см. `../crowler/pdn_parser/fetcher.py`, `crawler.py`). Это уберёт 20-секундные
простои на страницу и позволит безопасно поднять `PAID_MAX_PAGES`.
