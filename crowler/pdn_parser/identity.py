...
_LEGAL_NAME = re.compile(
    # юр-форма + название в кавычках: ООО «Ромашка»
    # \b перед формой обязателен: без него «АО» ловится внутри других слов —
    # ВАО/САО/ЮАО/ЦАО (округа Москвы) дали бы ложное юрлицо «АО ...».
    r"\b(?:ООО|ОАО|ЗАО|ПАО|АО)\s+[«\"][^»\"]{2,80}[»\"]"
    # юр-форма + название без кавычек: ООО Ромашка / АО Газпром нефть
    # (пробел, не \s — иначе имя «расползётся» через перенос строки)
    r"|\b(?:ООО|ОАО|ЗАО|ПАО|АО)\s+[А-ЯЁ][А-Яа-яЁё0-9 \-]{1,60}"
    # ИП Иванов И. И.
