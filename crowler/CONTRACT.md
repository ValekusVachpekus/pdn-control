...
| `server_is_cdn` | bool | IP принадлежит известному CDN/облаку (Cloudflare, Google, AWS/CloudFront, Fastly, Akamai …) по ASN или статическому списку диапазонов. Тогда `server_country` — страна **узла CDN**, а не обязательно место хранения ПДн: бэк не должен штрафовать вслепую. |
| `server_country_confidence` | enum | `high` — страна из базы и это не CDN; `low` — страна из базы, но IP за CDN (origin может быть в другой стране); `unknown` — страну определить не удалось. |
